...
app.get('/hotels/sort/pricing',(req,res)=>{
  let stype = req.query.pricing;
  let hotels_cpy = hotel.slice();
  if(stype==="low-to-high"){
    hotels_cpy.sort(sort_lth);
  }else{
    hotels_cpy.sort(sort_htl);
  }
  return res.json({hotels:hotels_cpy});
});



app.get('/hotels/sort/rating',(req,res)=>{
  let stype = req.query.rating;
  let hotels_cpy = hotel.slice();
  if(stype==="low-to-high"){
    hotels_cpy.sort(sort_lth_rating);
  }else{
    hotels_cpy.sort(sort_htl_rating);
  }
  return res.json({hotels:hotels_cpy});
});
...
app.get('/hotels/sort/reviews',(req,res)=>{
  let stype = req.query.reviews;
  let hotels_cpy = hotel.slice();
  if(stype==="least-to-most"){
    hotels_cpy.sort(sort_lth_review);
  }else{
    hotels_cpy.sort(sort_htl_review);
  }
  return res.json({hotels:hotels_cpy});
});
...
app.get('/hotels/filter/amenity',(req,res)=>{
  ...
  let filtered_hotels = hotel.filter((each_hotel)=>get_hotel_with_amenity(each_hotel,amenity));
  return res.json({hotels:filtered_hotels});
});

function get_hotel_with_country(each_hotel,country){
  return each_hotel.country.toLowerCase()===country;
};
...
app.get('/hotels/filter/country',(req,res)=>{
  ...
  let filtered_hotels = hotel.filter((each_hotel)=>get_hotel_with_country(each_hotel,country));
  return res.json({hotels:filtered_hotels});
});
